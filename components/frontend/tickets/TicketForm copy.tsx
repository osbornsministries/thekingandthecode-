...
import { processTicketPurchase } from '@/lib/actions/ticket/purchase-processor';
...
interface Props {
  dbPrices: any[];   // Using the Drizzle schema types here is best
  dbMethods: any[];
  dbDays: any[];     // Event days from DB
  dbSessions: any[]; // Event sessions from DB
}
...
export default function TicketForm({ dbPrices, dbMethods, dbDays, dbSessions }: Props) {
  ...
  const [formData, setFormData] = useState({
    ...
    fullName: '',
    phone: '',
    studentId: '',        // Added for student tickets
    institution: '',      // Added for student tickets
    institutionName: ''   // Added for "OTHER" institution option
  });
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      const submissionData = {
        ...formData,
        ticketType: selectedPriceConfig?.type || 'REGULAR',
        totalAmount,
        // Include student info only if it's a student ticket
        ...(isStudentTicket ? {
          studentId: formData.studentId,
          institution: formData.institution,
          institutionName: formData.institution === 'OTHER' ? formData.institutionName : null
        } : {})
      };

      const result = await processTicketPurchase(submissionData);
  ...
  const handleFinish = () => {
    ...
    if (createdTicketId) router.push(`/ticket/${createdTicketId}`);
  };
  ...
  return (
    <div ref={containerRef} className="w-full max-w-lg mx-auto relative">
      <div className="bg-white/40 backdrop-blur-xl border border-white/50 rounded-[40px] p-8 md:p-10 shadow-2xl relative overflow-hidden min-h-[500px] flex flex-col">
          ...
          {step === 6 && (
            <StepDetails
              ...
              summary={{
                ...
                ticketType: selectedPriceConfig?.name || 'N/A',
                quantity: formData.quantity,
                total: totalAmount
              }}
            />
        ...
        {/* Error & Buttons */}
        {errorMessage && (
          <div className="mb-4 p-3 bg-red-100 border border-red-200 rounded-xl flex items-center gap-2 text-red-700 text-sm">
        ...
        <div className="mt-8 flex gap-4">
          {step > 1 && (
            <button 
              onClick={handlePrev} 
              disabled={loading}
              className="px-6 py-4 rounded-xl border border-gray-400 hover:bg-white/50 transition disabled:opacity-50"
            >
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 font-bold text-white py-4 rounded-xl transition-all shadow-lg bg-gradient-to-r from-[#602324] to-[#cf635f] hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? (
      ...
      {showSuccess && <SuccessModal onClose={handleFinish} />}
    </div>
  );
}