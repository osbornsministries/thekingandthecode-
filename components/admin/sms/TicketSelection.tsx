...
import { Search, Filter, X, Check, Phone, User, Calendar, Clock, MessageSquare } from 'lucide-react';
...
interface TicketSelectionProps {
  ...
  // Add new props for SMS integration
  onSendMessages?: (selectedTicketIds: number[]) => void;
  isSending?: boolean;
...
export function TicketSelection({
  ...
  // Auto-select tickets when a template is selected and no tickets are selected
  useEffect(() => {
    if (selectedTemplateName && selectedTickets.length === 0 && filteredTickets.length > 0) {
      // Auto-select 10 tickets or all if less than 10
      const ticketsToSelect = filteredTickets
        .slice(0, 10)
        .map(t => t.id);
  ...
  }, [selectedTemplateName, filteredTickets.length]);
  ...
  const handleSendMessages = () => {
    if (onSendMessages && selectedTickets.length > 0) {
      onSendMessages(selectedTickets);
    }
  };
  ...
  // Calculate SMS cost estimation (assuming 1 SMS per message)
  const smsCostEstimate = selectedTickets.length;
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                ...
                {smsCostEstimate > 0 && ` • ${smsCostEstimate} SMS unit(s) estimated`}
              </CardDescription>
            </div>
            <div className="flex gap-2">
              ...
              {onSendMessages && selectedTickets.length > 0 && (
                <Button 
                  variant="default" 
                  size="sm" 
                ...
                >
                  {isSending ? (
                    ...
                    <>
                      ...
                      Send SMS ({selectedTickets.length})
                    </>
                  )}
                </Button>
              )}
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-blue-800">
                  Ready to send with template: <span className="font-bold">{selectedTemplateName}</span>
                </span>
              </div>
          ...
          <div className="flex gap-2">
            ...
            {onSendMessages && selectedTickets.length > 0 && (
              <Button
                ...
                disabled={isSending}
              >
                {isSending ? 'Sending...' : `Send SMS (${selectedTickets.length})`}
              </Button>
            )}
          </div>
        </div>
...
function TicketItem({ ticket, session, day, isSelected, onToggle }: TicketItemProps) {
  ...
  const formatPhoneForSMS = (phone: string) => {
    ...
    // Format for display but keep original for SMS sending
    if (phone.startsWith('255') && phone.length === 12) {
      return `+${phone.slice(0, 3)} ${phone.slice(3, 6)} ${phone.slice(6, 9)} ${phone.slice(9)}`;
    }
    
    // If starts with 0, show +255 format
    if (phone.startsWith('0') && phone.length === 10) {
      return `+255 ${phone.slice(1, 4)} ${phone.slice(4, 7)} ${phone.slice(7)}`;
    }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-sm">{formatPhoneForSMS(ticket.purchaserPhone)}</span>
              </div>
            </div>
